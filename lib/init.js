...
function hasExistingEtapa(root) {
  return (
...
function initProject(root, options) {
  ...
  const etapaDetected = hasExistingEtapa(targetRoot);

  console.log("");
  console.log(t("init.welcome"));

  return { root: targetRoot, isUpgrade, etapaDetected };
}
...
function cmdInit(args) {
  ...
  if (options.withEtapa) {
    const etapa = require("./etapa");
    etapa.install(root, {});
  } else if (result.etapaDetected) {
    console.log("");
    console.log(t("etapa.primitiveDetected"));
    console.log("  Run 'trackops etapa install' to upgrade to managed ETAPA.");
  }
}
