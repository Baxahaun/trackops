...
function scanProject(root) {
  ...
  const readmePath = fs.existsSync(path.join(root, "README.md"))
    ? path.join(root, "README.md")
    : fs.existsSync(path.join(root, "README.mdx"))
  ...
  const title = pkg?.displayName || pkg?.name || path.basename(root);
  const testCommands = unique([
    pkg?.scripts?.test ? "npm test" : "",
    fs.existsSync(path.join(root, "pytest.ini")) ? "pytest" : "",
    fs.existsSync(path.join(root, "Cargo.toml")) ? "cargo test" : "",
    fs.existsSync(path.join(root, "go.mod")) ? "go test ./..." : "",
  ]);
  const buildCommands = unique([
    pkg?.scripts?.build ? "npm run build" : "",
    fs.existsSync(path.join(root, "Dockerfile")) ? "docker build ." : "",
  ]);
  const services = unique([
    ...inferServicesFromDependencies(pkg),
    ...inferServicesFromEnv(envExample),
  ]);
  const gitRemote = git(["remote", "get-url", "origin"], root) || null;
...
function detectLegacyBootstrap(root, control) {
  ...
  const genesisContent = readText(path.join(root, "genesis.md"));
  return {
    version: 1,
