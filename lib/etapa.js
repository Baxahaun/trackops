...
const TEMPLATES_DIR = path.join(__dirname, "..", "templates", "etapa");
const SKILLS_TEMPLATES_DIR = path.join(__dirname, "..", "templates", "skills");
const ETAPA_VERSION = require("../package.json").version;
...
function install(root, options) {
  ...
  if (config.isEtapaInstalled(control)) {
    console.log(t("etapa.alreadyInstalled", { version: config.getEtapaVersion(control) }));
    return;
  }
  ...
  control.meta.etapa = {
    installed: true,
    version: ETAPA_VERSION,
    installedAt: nowIso(),
    skills: [],
  };
  config.saveControl(root, control);

  console.log(t("etapa.installed", { version: ETAPA_VERSION }));
...
function status(root) {
  ...
  if (!config.isEtapaInstalled(control)) {
    console.log(t("etapa.notInstalled"));
    return;
  }

  const etapa = control.meta.etapa;
  console.log(`ETAPA v${etapa.version}`);
  console.log(`  Installed: ${etapa.installedAt}`);
  console.log(`  Skills: ${(etapa.skills || []).join(", ") || "none"}`);
...
function configure(root, args) {
  ...
  // Parse --phases and --locale from args
...
function upgrade(root) {
  ...
  if (!config.isEtapaInstalled(control)) {
    console.log(t("etapa.notInstalled"));
    console.log("Run 'trackops etapa install' first.");
    return;
  }
  ...
  control.meta.etapa.version = ETAPA_VERSION;
  config.saveControl(root, control);
  console.log(t("etapa.upgraded", { version: ETAPA_VERSION }));
}
